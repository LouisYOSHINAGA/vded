...
**L1 / L2 のバッジ**の役割はタブごとに違います。ALL LAYERS ではその PART の
**LINK の接続／解除**（押すと両方のバッジが点灯し、括弧線が出ます）。DIALS では
そのレイヤを PART EDIT で開きます。single channel mode では LINK を解除できないので、
ALL LAYERS のバッジは点灯したまま操作できなくなります。
