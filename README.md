...
- **FOLLOW** を ON にすると、再生位置に追従します。ページの自動切り替えに加え、
  **1 拍（4 ステップ）遅れで次ページを順次先読み表示**します（5 ステップ目で 17 番、
  6 ステップ目で 18 番…）。先読み中のセルは淡く表示され、クリックすると表示している
  ステップの方を編集します。最終ページでは 1 ページ目が戻ってきます。
  OFF にすれば表示は固定され、別のページを鳴らしながら任意のページを編集できます
