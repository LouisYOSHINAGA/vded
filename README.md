...
- **SKIN** — 配色（THEME タブ）と、フォント・表示倍率（TEXT タブ）。設定は保存されます。
...
- 拍の区切りは 4 ステップごとの**余白**で示します（枠線で強調すると、そのステップだけ
  別の状態に見えてしまうため）
...
- `TEMPO` / `SWING` はダイヤル（実機のパネルに合わせています）。数値をクリックすれば直接入力も可
- `GATE`（ノート長 ms）/ `LEN`（1–16）
...
**TEXT** タブでフォントと表示倍率を選びます。フォントは Meiryo / Consolas（既定）、
