...
- **FOLLOW** を ON にすると、再生位置に追従します。ページの自動切り替えに加え、
  隣の **LAG** を ON にすると **次ページを先読み表示**します。再生位置の 1 拍
  （4 ステップ）後ろから、**4 個まとめて**入れ替わります（8 ステップ目で 1 拍目が
  17–20 番、12 ステップ目で 2 拍目が 21–24 番）。ページ最後の 1 拍に入ってから
  切り替わる分は、直後のページ送りにまとめられます（1 ステップ差で 2 回描き変わると
  ちらつくため）。先読み中のセルは淡く表示され、
