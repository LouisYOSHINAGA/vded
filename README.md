...
- レーン右端の **SCALE** は、そのパートの 16 ステップ分の velocity をまとめて倍率で掛けます。
  掛ける対象のフェーダーと同じ向きになるよう縦型にしてあります（スライダを動かしただけでは
  変化せず、**APPLY** を押したときに適用されます）
...
MIDI MONITOR は送受信したメッセージを**表**で表示します（方向 / 種別 / Ch / No. / 値 / 対象）。
CC が連続して流れるときは、桁が揃っていないと読めないためです。
