...
- **SKIN** — 配色（THEME タブ）と、言語・フォント・表示倍率（TEXT タブ）。設定は保存されます。
...
- 拍の区切りは 4 ステップごとの**細い縦線**で示します（枠線でパッドを強調すると、その
  ステップだけ別の状態に見えてしまうため。パッドの幅は 16 個すべて同一です）
...
- **LINK L1+2** — 両レイヤを同時に編集し、`L1+2` の CC 1 通で送信します。ON のあいだは
  2 つのレイヤが 1 つの枠で囲まれ、あいだに鎖のマークが出ます（ALL LAYERS タブでも
  L1 / L2 が括弧で結ばれます）。
...
- **SAVE** はブラウザ内（localStorage）への保存、**EXPORT .JSON / IMPORT .JSON** は
  ファイルとしての書き出し・読み込みです。
...
**TEXT** タブで言語・フォント・表示倍率を選びます。フォントは Meiryo / Consolas（既定）、
Consolas 全面、System UI、Condensed、Serif。ラベルの字送りは選んだ書体に合わせて自動調整
されます。表示倍率は 70 / 80 / 90 / 100 / 110 / 125%。
