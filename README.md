...
- **SKIN** — スキン（4 種）、フォント（5 種）、表示倍率（90–125%）。設定は保存されます。
...
| スキン | 内容 |
| --- | --- |
| **Vermilion** | 既定。実機のパネルに合わせた暗いチャコール＋バーミリオン |
| **Carbon** | グラファイト＋アンバー。彩度をさらに落とした構成 |
| **Blueprint** | 濃紺＋アイスブルー |
| **Paper** | ライトスキン。明るい部屋やスクリーン共有向け |

フォントは Meiryo / Consolas（既定）、Consolas 全面、System UI、Condensed、Serif から選べます。
ラベルの字送りは選んだ書体に合わせて自動調整されます。表示倍率は 90 / 100 / 110 / 125%。

配色は 1 つのアクセント色を基本にしています。6 パートは実機の LED と同じく同一色で、
行の識別は番号チップ・縞・選択行のハイライトで行います。レイヤ 2 だけはアクセントから
少しずらした色（既定ではオーカー）を使い、レイヤ 1 と見分けられるようにしています。
