...
6 パート × 16 ステップを一覧できるグリッド。パターンは **4 ページ（最長 64 ステップ）**まで
伸ばせます。画面には常に 1 ページ（16 ステップ）を表示し、ページで切り替えます。
...
- パート番号はクリックで選択、**ダブルクリックで PART EDIT** を開きます。
  パート名は**ダブルクリックでその場で改名**できます（入力した通りに表示されます —
  小文字も記号も使えます）
...
- レーン右端の **SCALE** は、そのパートの velocity をまとめて倍率で掛けます。
...
- `LEN`（パターン長 1–64）と **PAGE 1–4** はグリッド直上に並べています（長さとページは
  同じことを別の角度から言っているため）。
  ボタンの **ドット**はそのページにステップが置かれていることを、**淡い表示**は `LEN` の外側で
  まだ再生されないことを表します（外側のページも先に書き込んでおけます）
- **FOLLOW** を ON にすると、再生位置に追従します。ページの自動切り替えに加え、
  **次ページを先読み表示**します。切り替えは **4 ステップ（1 拍）のブロック単位**で、
  4 個まとめて入れ替わります。何ステップ遅れで切り替えるかは隣の **LAG** で選べます
  （`Off` / `4` / `8`、既定は `8`）。LAG 8 なら 12 ステップ目で 1 拍目が 17–20 番に
  入れ替わります。最後の 1 拍に入ってから切り替わる分は、直後のページ送りに
  まとめられます（1 ステップ差で 2 回描き変わるとちらつくため）。
  先読み中のセルは淡く表示され、
  クリックすると**表示している方のステップ**を編集します。最終ページでは 1 ページ目が
  戻ってきます。`Off` を選ぶとページが切り替わるまで表示は動きません。
  FOLLOW 自体を OFF にすれば表示は完全に固定され、別のページを鳴らしながら任意の
  ページを編集できます
...
MIDI MONITOR は送信したメッセージを**表**で表示します（Ch / CC / 値 / 対象）。
数値の 3 列は 1 つのまとまりとして読めるよう詰めてあり、文字列である対象だけ左寄せです。
送るのは CC と PANIC のノートオフだけなので種別の列は置かず、PANIC はその旨を対象欄に
書いています。対象名が長いときは横スクロールします。
...
どちらのタブでも **PART 名はクリックでそのパートを選択**（シーケンサの編集対象が変わります）、
**ダブルクリックで PART EDIT を開きます**。

どちらのタブでも **L1 / L2 のバッジ**はその PART の **LINK の接続／解除**です。
押すと両方のバッジが点灯し、左に括弧線が出ます。single channel mode では LINK を
解除できないので、バッジは点灯したまま操作できなくなります。

### MEMO

自由記述の覚書。ワークスペースと一緒に保存されます（プリセットには入りません）。
このキットの用途、実機のどのツマミを回したか、次回の設定など、機械が覚えていられない
ことを書いておく場所です。
...
- 各プリセットの左端の**グリップ**を掴むとドラッグで**並べ替え**できます（矢印キーでも可）。
  並び順はプリセット本体と一緒に保存されます。検索で絞り込んでいるあいだも、
  表示されているカードの隣へ落とす形で並べ替えられます。
...
ライト 8 種・ダーク 8 種を用意しています。

| ライト | ダーク |
| --- | --- |
| **Paper**（既定）温かみのある紙色＋朱 | **Vermilion** 実機のパネルに近い炭色 |
| **Linen** 生成りの麻色＋琥珀 | **Ember** 暖かい炭色＋橙 |
| **Clay** 陶土色＋赤茶 | **Carbon** 黒鉛＋琥珀 |
| **Sage** 淡い緑灰＋深緑 | **Basalt** 玄武岩＋青緑 |
| **Studio** 中庸の灰色＋藍 | **Slate** 石板色＋淡青 |
| **Graph** 方眼紙の白緑＋深い青緑 | **Blueprint** 濃紺＋氷青 |
| **Porcelain** 磁器の白＋鉄色 | **Moss** 苔緑＋黄緑 |
| **Daylight** 純白＋青緑 | **Plum** 李色＋藤 |

ダーク系の下地は黒ではなく**灰寄り**にしてあります。中間灰のパネルの背後に純黒を置くと
画面に穴が空いたように見え、パネルの縁が見えなくなるためです。
...
CUSTOM は **8 枠**あり、それぞれ別の配色を持てます。枠を押すと切り替わり、
**「編集」** で選択中の枠の基準色を直接指定できます（枠をダブルクリックしても編集に入ります）。
各枠は既存のスキンから始まるので、白紙から作る必要はありません。
...
パネルには説明文を置かず、ⓘ アイコンにマウスを重ねたときだけ表示します。**クリックすると
固定表示**になり、マウスを離しても読み続けられます（別の場所をクリック、または Esc で閉じます）。
実機側の設定やチャンネルモードのような長い説明は **FUNC** / **MIDI MAP** タブにまとめてあります。
タブは下のパネルと地続きになっていて、選択中のタブがパネルと同じ面につながります。
**タブ自体はドラッグで並べ替えられます**（キーボードは Alt + 矢印キー）。並びは保存されます。
